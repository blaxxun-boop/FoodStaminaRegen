...
public class FoodStaminaRegen : BaseUnityPlugin
{
	...
	private class ReadFoodConfigs
	{
		...
		private static void Postfix()
		{
			...
			List<ItemDrop.ItemData.SharedData> items = ObjectDB.instance.m_items.Where(i => i.GetComponent<ItemDrop>()).Select(i => i.GetComponent<ItemDrop>().m_itemData.m_shared).ToList();
			staminaRegen = items.Where(item => item.m_itemType == ItemDrop.ItemData.ItemType.Consumable && item.m_foodStamina > 0)
				.ToDictionary(item => item.m_name, item => mod.config("2 - Food", regex.Replace(english.Localize(item.m_name), ""), (float)Math.Round(item.m_foodStamina * 0.03f, 2), new ConfigDescription("", null, new ConfigurationManagerAttributes { DispName = regex.Replace(Localization.instance.Localize(item.m_name), "") })));
		}
	}
	...
	private class FoodDescPatch
	{
		...
		{
			if (staminaRegen.TryGetValue(item.m_shared.m_name, out ConfigEntry<float> regen))
			{
